...
from utils.batch_inference import batch_inference
...
def asyncRunModelChat(case, num_gpus, token, dataset, window_size, hf_dataset, split, zeroshot):
    ...
    log_path, _ = open_record_directory(dataset, f"finetuned-case{case}", window_size)
        
    with concurrent.futures.ProcessPoolExecutor(max_workers=num_gpus) as executor:
    # Create a dictionary to map each future to its corresponding index
        future_to_index = {
            executor.submit(batch_inference, dataset_parts[i], case, log_path, devices[i], token, dataset, window_size, zeroshot): i
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fine-tuned Configuration")
    ...
    results = asyncRunModelChat(case, num_gpus, token, dataset, window_size, hf_dataset, split, zeroshot=False)
